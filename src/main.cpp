...
#if defined(NDEBUG)
# error "Chopcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x36e1cf51c3b52419ac15fd3301df7ae74c16a62ffdded16aa5435286be69a28f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Chopcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Chopcoin Signed Message:\n";
...
{
    // Chopcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Chopcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Chopcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Chopcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Chopcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chopcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x36e1cf51c3b52419ac15fd3301df7ae74c16a62ffdded16aa5435286be69a28f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ALL HAIL THE MIGHTY CHOPCOIN";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("121284710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1486085638;
        ...
        assert(block.hashMerkleRoot == uint256("0x5e8700ca560f7020fd27a2d3492146c33656f2f1aec0f7d8be42a2b4bbe37d58"));
        block.print();
	printf("%s\n", hashGenesisBlock.ToString().c_str());
	printf("%s\n", hash.ToString().c_str());
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Chopcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChopcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChopcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChopcoinMiner(CWallet *pwallet)
{
    printf("ChopcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chopcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ChopcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChopcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChopcoinMiner, pwallet));
}
